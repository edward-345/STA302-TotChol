...
pre_coef_table <- as.data.frame(coef(summary(model)))

pre_confints <- confint(model)

pre_coef_table$`2.5 %` <- round(pre_confints[, 1], 4)
pre_coef_table$`97.5 %` <- round(pre_confints [, 2], 4)

knitr::kable(pre_coef_table, digits = 4, caption = "Preliminary Model Coefficients")
```
...
$$Y^{(\lambda)} = \frac{Y^\lambda - 1}{\lambda}, \quad \lambda = 0.1414\tag{1}$$
...
\hat{\beta}_{Height}x_{Height} + \hat{\beta}_{BPSysAve}x_{BPSysAve} + \hat{\beta}_{BPDiaAve}x_{BPDiaAve} + \hat{\epsilon}\tag{2}
$$
```{r,echo=FALSE,message=FALSE}
...
$$
$$
 \hat{\beta}_{PhysActiveDays}x_{PhysActiveDays}+\hat{\beta}_{SmokeNowYes}x_{SmokeNowYes}+\hat{\epsilon}\tag{3}
$$


### Diagnostic Plots of Preliminary Model
```{r,echo=FALSE,fig.cap="Residual Plots of Preliminary Model", out.width="100%"}
...
```{r,echo=FALSE,fig.cap="Residual Plots of Transformed Model"}
...
```{r,echo=FALSE,fig.cap="Residual Plots of Transformed Model",out.width="100%"}
