...
leverage_points_index <- which(leverage > high_lev)

leverage_frame <- p.BXCX.frame %>% 
  filter(row_number() %in% leverage_points_index)
#Rows 410, 774, 1288 have misinputs in BPDiaAve, entry says 0

#FINDING OUTLIERS
st.residuals <- rstandard(reduced.model) 

#Definition of outlier on page 9 of Mar 19th slides
outliers_index <- which(abs(st.residuals)>4)

outlier_frame <- p.BXCX.frame %>% 
  filter(row_number() %in% outliers_index)
#This outlier point is NOT AND INFLUENTIAL POINT

#FILTERED DATAFRAME EXCLUDING MISINPUT VARIABLES
filtered_data <- p.BXCX.frame %>% 
  filter(!(row_number() %in% c(410, 774, 1288)))
```
...
best_subset_p.BXCX <- regsubsets(pb.TotChol~., data=p.BXCX.frame,nvmax=7,                  nbest=1,really.big=TRUE,method="exhaustive")
