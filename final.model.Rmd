...
nhanes_data <- nhanes_filtered %>% 
  ...
  na.omit() %>%
  dplyr::filter(
    Height > 0, Weight > 0, BPDia1 > 10, BPDia2 > 10, BPDia3 > 10, BPSys1 > 10, BPSys2 > 10, BPSys3 > 10, TotChol > 0) %>% dplyr::select(Height, Age, Weight, BPSysAve, BPDiaAve,
                TotChol, SmokeNow, PhysActiveDays)
...
leverage_points_index
rownames(p.BXCX.frame)[leverage_points_index]


## Check if the absolute values of the standardized residuals are greater than 3
st.residuals <- rstandard(reduced.model) ## standardized residuals
st.residuals

## Outliers
outliers_index <- which(abs(st.residuals)>3)
outliers_index
